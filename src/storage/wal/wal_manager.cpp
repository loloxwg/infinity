    ...
    while (running_.load()) {
        ...
        for (const auto &entry : que2_) {
            ...
            if (entry->cmds_.empty()) {
                Error<StorageException>(Format("WalEntry of txn_id {} commands is empty", entry->txn_id_));
            }
            ...
            LOG_TRACE(Format("WalManager::Flush done writing wal for txn_id {}, commit_ts {}", entry->txn_id_, entry->commit_ts_));
...
void WalManager::ReplayWalEntry(const WalEntry &entry) {
    for (const auto &cmd : entry.cmds_) {
        LOG_TRACE(Format("Replay wal cmd: {}, commit ts: {}", WalManager::WalCommandTypeToString(cmd->GetType()).c_str(), entry.commit_ts_));
        switch (cmd->GetType()) {
            case WalCommandType::CREATE_DATABASE:
